...
void Lexer::handleOperator(std::ifstream &f, std::string &tok, std::vector<Token> &lexes, char &ch, FilePosition &pos) {
    flushTok(lexes, tok, pos);
    tok.push_back(ch);
    f.seekg(1, std::fstream::cur);
    ch = f.peek();
...
std::vector<Token> Lexer::tokenize(std::string filename) {
    ...
    if (!f.is_open()) {std::cout << "Error opening file" << std::endl;}
    std::string tok;
    char c;
